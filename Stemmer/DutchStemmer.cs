...
namespace Annytab
{
  ...
  public class DutchStemmer : AbstractStemmer
  {
    ...
    public int[] CalculateR1R2(char[] characters)
    {
    ...
    public override string GetSteamWord(string word)
    {
      ...
      else if (word.EndsWith("ene"))
      {
        ...
        if (strR1.EndsWith("ene") && word.Length > 3 && word.EndsWith("gemene") == false &&
      ...
      {
        // Delete if in R1 and preceded by a valid en-ending, and then undouble the ending
        if (strR1.EndsWith("en") && word.Length > 2 && word.EndsWith("gemene") == false &&
      ...
      else if (word.EndsWith("se"))
      {
        ...
        if (strR1.EndsWith("se") && word.Length > 2 && word[word.Length - 3] != 'j' &&
            IsVowel(word[word.Length - 3]) == false)
          word = word.Remove(word.Length - 2);
      }
      ...
      {
        // Delete if in R1 and preceded by a valid s-ending
        if (strR1.EndsWith("s") && word.Length > 1 && word[word.Length - 2] != 'j' &&
      ...
      if (strR2.EndsWith("heid") && word.Length > 4 && word[word.Length - 5] != 'c')
      {
        ...
        if (strR1.EndsWith("enheid") && word.Length > 2 && word.EndsWith("gemene") == false &&
            IsVowel(word[word.Length - 3]) == false)
        {
          word = word.Remove(word.Length - 2);
      ...
      if (word.Length <= 3) return word.ToLowerInvariant();
      // Get CVD

      var v = word.Substring(word.Length - 3, 2);
      var d = word[word.Length - 1];

      if (d != 'I' && IsVowel(word[word.Length - 4]) == false && IsVowel(d) == false &&
          (v == "aa" || v == "ee" || v == "oo" || v == "uu"))
