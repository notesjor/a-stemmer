  ...
  public class FrenchStemmer : AbstractStemmer
  {
      ...
      _endingsStep1 = new[]
                      {
                        ...
                        "utions", "ements", "amment", "emment", "ances", "iqUes", "ismes", "ables", "istes", "ateur",
                        "ation", "logie", "usion",
                        "ution", "ences", "ement", "euses", "ments", "ance", "iqUe", "isme", "able", "iste", "ence",
                        "ités", "ives", "eaux",
                        "euse", "ment", "eux", "ité", "ive", "ifs", "aux", "if"
                      };
      _endingsStep2A = new[]
                       {
                         "issantes", "issaIent", "issante", "issions", "issants", "iraIent", "issant", "irions",
                         "issiez", "issons", "issais", "issait", "issent", "iriez", "isses", "irais", "iront", "irait",
                         "issez", "irons",
                         "irent", "irez", "iras", "îmes", "îtes", "isse", "irai", "ira", "ies", "ît", "is", "ir", "ie",
                         "it", "i"
                       };
      _endingsStep2B = new[]
                       {
                         "eraIent", "assions", "assiez", "erions", "assent", "erait", "èrent", "erons",
                         "antes", "eriez", "asses", "eront", "erais", "aIent", "asse", "ants", "ante", "erai", "eras",
                         "âtes", "erez",
                         "âmes", "ions", "ait", "ant", "era", "iez", "ées", "ais", "ez", "ât", "ai", "er", "as", "és",
                         "ée", "é", "a"
                       };
    ...
    public override string GetSteamWord(string word)
    {
      ...
      for (var i = 0; i < chars.Length; i++)
      {
        if (i == 0)
        {
          if (chars.Length > 1 && chars[i] == 'y' && IsVowel(chars[i + 1]))
            chars[i] = 'Y';
        }
        else if (i == charCount)
        {
        ...
        switch (end)
        {
          ...
          case "ement":
            ...
            if (strRV.EndsWith(end))
            {
              ...
              else if (strR2.EndsWith("abl" + end) || strR2.EndsWith("iqU" + end))
                // If preceded by abl or iqU, delete if in R2
                word = word.Remove(word.Length - 3);
              else if (strRV.EndsWith("ièr" + end) || strRV.EndsWith("Ièr" + end))
          ...
          case "issement":
            ...
            if (word.Length > end.Length && strR1.EndsWith(end) &&
          ...
          case "ment":
            ...
            if (strRV.Length > end.Length && strRV.EndsWith(end) && IsVowel(strRV[strRV.Length - end.Length - 1]))
            {
              word = word.Remove(word.Length - end.Length);
              wordIsAltered = true;
            }
      ...
      if (wordIsAltered == false)
      {
        ...
        if (strRV.EndsWith("Ière") || strRV.EndsWith("ière"))
        {
          // Replace with i
          word = word.Remove(word.Length - 4);
          word += "i";
        }
        else if (strRV.EndsWith("Ier") || strRV.EndsWith("ier"))
        {
          // Replace with i
          word = word.Remove(word.Length - 3);
          word += "i";
        }
        else if ((strRV.EndsWith("sion") || strRV.EndsWith("tion")) && strR2.EndsWith("ion"))
        {
          // Delete ion
          word = word.Remove(word.Length - 3);
        }
          ...
          word = word.Remove(word.Length - 1);
        else if (strRV.EndsWith("ë") && word.EndsWith("guë"))
          word = word.Remove(word.Length - 1);
      }
      // **********************************************
      ...
      if (word.EndsWith("eill") || word.EndsWith("ell") || word.EndsWith("ett") || word.EndsWith("onn") ||
          word.EndsWith("enn"))
        word = word.Remove(word.Length - 1);
    ...
    private int[] CalculateR1R2RV(char[] characters)
    {
      ...
      // Calculate RV
      if (characters.Length > 3 && ((IsVowel(characters[0]) && IsVowel(characters[1]))
      ...
      else
      {
        ...
        {
          if (!IsVowel(characters[i])) continue;
          // Set the rV index
          rV = i + 1;
          break;
        }
