...
namespace Annytab
{
  ...
  public class SpanishStemmer : AbstractStemmer
  {
    ...
    public SpanishStemmer()
    {
      ...
      _endingsStep0 = new[] {"selas", "selos", "sela", "selo", "nos", "los", "les", "las", "se", "le", "lo", "me", "la"};
      _endingsStep1 = new[]
                      {
                        "amientos", "imientos", "imiento", "amiento", "uciones", "aciones", "adoras", "adores", "idades",
                        "amente", "encias", "ancias", "logías", "ución", "encia", "ancia", "antes", "mente", "istas",
                        "ibles", "ables", "ación", "anzas", "adora",
                        "ismos", "logía", "ivos", "ante", "osas", "osos", "ismo", "icas", "idad", "ista", "icos", "ible",
                        "anza", "able", "ivas", "ador", "osa",
                        "oso", "iva", "ivo", "ica", "ico"
                      };
      _endingsStep2a = new[] {"yamos", "yendo", "yeron", "yais", "yes", "yas", "yan", "yen", "ye", "yó", "ya", "yo"};
      _endingsStep2b = new[]
                       {
                         "iésemos", "iéramos", "iríamos", "eríamos", "aríamos", "aremos", "ábamos", "iríais", "aríais",
                         "ásemos", "isteis", "asteis", "ieseis", "eremos", "ierais", "áramos", "iremos", "eríais",
                         "arías", "aréis", "aseis", "arais",
                         "íamos", "abais", "iesen", "erían", "ieses", "ieran", "ieras", "eréis", "arían", "irían",
                         "irías", "iréis", "iendo", "ieron",
                         "erías", "irás", "aron", "irán", "asen", "ando", "aran", "aría", "aban", "abas", "iste", "idos",
                         "iese", "arás", "adas", "idas",
                         "arán", "iera", "ería", "aras", "erás", "erán", "ases", "amos", "imos", "emos", "aste", "íais",
                         "iría", "ados", "iré", "ido", "ían",
                         "ado", "aré", "ase", "ará", "áis", "eré", "ara", "erá", "ida", "ada", "aba", "éis", "irá",
                         "ías",
                         "an", "es", "id", "ed", "ad", "ía",
    ...
    public override string GetSteamWord(string word)
    {
      ...
      foreach (var end in _endingsStep0.Where(end => word.EndsWith(end)))
      {
        ...
        else if (strRV.EndsWith("iendo" + end) || strRV.EndsWith("ando" + end) ||
                 strRV.EndsWith("ar" + end) || strRV.EndsWith("er" + end) ||
                 strRV.EndsWith("ir" + end))
        {
          // Delete the ending
          word = word.Remove(word.Length - end.Length);
        }
        else if (strRV.EndsWith("yendo" + end) && word.EndsWith("uyendo" + end))
        {
          // Delete the ending
          word = word.Remove(word.Length - end.Length);
        }
        ...
        switch (end)
        {
          ...
          case "amente":
            ...
            if (strR1.EndsWith(end))
            {
              ...
              if (strR2.EndsWith("iv" + end) || strR2.EndsWith("os" + end) ||
                  strR2.EndsWith("ic" + end) || strR2.EndsWith("ad" + end))
              {
                word = word.Remove(word.Length - 2);
          ...
          case "mente":
            ...
            if (strR2.EndsWith(end))
            {
              ...
              if (strR2.EndsWith("ante" + end) || strR2.EndsWith("able" + end) ||
      ...
      if (endingRemovedStep1 == false && doStep_2B)
      {
        foreach (var end in _endingsStep2b.Where(end => word.EndsWith(end)))
        {
          if (end == "en" || end == "es" || end == "éis" || end == "emos")
          {
            ...
            if (!strRV.EndsWith(end)) continue;
            word = word.Remove(word.Length - end.Length);
      ...
      foreach (var end in _endingsStep3.Where(end => word.EndsWith(end)))
      {
        if (end == "é" || end == "e")
        {
          ...
          if (strRV.EndsWith("u" + end) && word.EndsWith("gu"))
            word = word.Remove(word.Length - 1);
