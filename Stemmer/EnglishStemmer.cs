  ...
  public class EnglishStemmer : AbstractStemmer
  {
    ...
    public EnglishStemmer()
    {
      ...
      _step2Replacements = new[,]
                           {
                             ...
                             {"ousness", "ous"}, {"biliti", "ble"},
                             {"tional", "tion"}, {"lessli", "less"}, {"fulli", "ful"}, {"entli", "ent"},
                             {"ation", "ate"},
                             {"aliti", "al"}, {"iviti", "ive"}, {"ousli", "ous"}, {"alism", "al"},
                             {"abli", "able"}, {"anci", "ance"}, {"alli", "al"}, {"izer", "ize"}, {"enci", "ence"},
      ...
      _step4Replacements = new[]
                           {
                             "ement", "ment", "ence", "able", "ible", "ance", "ism", "ent", "ate", "iti", "ant", "ous",
                             "ive", "ize", "ion", "ic", "er", "al"
                           };
      _exceptions = new[,]
                    {
                      {"skis", "ski"}, {"skies", "sky"}, {"dying", "die"}, {"lying", "lie"}, {"tying", "tie"},
                      {"idly", "idl"}, {"gently", "gentl"}, {"ugly", "ugly"},
                      {"early", "early"}, {"only", "only"}, {"singly", "singl"}, {"sky", "sky"}, {"news", "news"},
                      {"howe", "howe"}, {"atlas", "atlas"}, {"cosmos", "cosmos"}, {"bias", "bias"}, {"andes", "andes"}
                    };
    ...
    public int[] CalculateR1R2(string word)
    {
    ...
    public override string GetSteamWord(string word)
    {
      ...
      if (chars.Length > 1)
      {
        ...
        {
          foreach (var t in Vowels.Where(t => chars[i] == 'y' && chars[i - 1] == t))
            chars[i] = 'Y';
        }
      }
      ...
      for (var i = 0; i < 6; i++)
      {
        if (_step1Replacements[i, 0] == "eedly" && strR1.EndsWith("eedly"))
        {
          word = word.Length >= 2 ? word.Remove(word.Length - 2, 2) : word;
          break;
        }
        if (_step1Replacements[i, 0] == "eed" && strR1.EndsWith("eed"))
        {
          word = word.Length >= 1 ? word.Remove(word.Length - 1, 1) : word;
          break;
        }
        if (!word.EndsWith(_step1Replacements[i, 0])) continue;
      ...
      for (var i = 0; i < 24; ++i)
      {
        ...
        if (!word.EndsWith(_step2Replacements[i, 0])) continue;
      ...
      for (var i = 0; i < 9; ++i)
      {
        if (!strR1.EndsWith(_step3Replacements[i, 0])) continue;
